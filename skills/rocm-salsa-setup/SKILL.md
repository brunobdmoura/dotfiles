---
name: rocm-salsa-setup
description: Set up Salsa remotes for a ROCm package. tracking branch for `ubuntu/stonking`.
---

# Set up Salsa remotes for a ROCm package

Configure the standard Salsa git remotes for a ROCm Debian package and set the
tracking branch for `ubuntu/stonking`.
...
### 4 — Checkout `ubuntu/stonking`

```bash
git checkout ubuntu/stonking
```
...
```bash
git push -u salsa-bruno ubuntu/stonking
```
...
```bash
git branch --set-upstream-to=salsa-bruno/ubuntu/stonking
```
